...
def mcdint(et):
    ...
    # gw: Gauss Width?
    # gn: Gauss UNKNOWN
    gc = np.array([0.78196667, -1.0662020, 1.1924180])
    gw = np.array([0.52078740, 0.51345700, 0.40779830])
    gn = np.array([0.37286910, 0.039775528, 0.037766505])

    res = precomputation()

    loget = np.log10(et)
    pos = (loget - np.log10(ET0)) / STEP + 1
    j = int(np.floor(pos))
    pos = pos - j
    ...
    for j in range(3):
        z = (loget - gc[j]) / gw[j]
        gaufact += gn[j] * np.exp(-z * z / 2.0)

    value = VALUE0 * (et / ET0) ** BEKI * (
        1.0 + A * et ** B) * np.exp(-et) * gaufact * (1.0 + resfact)
    return value
...
def xsdskb(EAR, NE, PARAM, IDT):
    ...
    GAUSS = np.array([0.236926885, 0.478628670, 0.568888888,
